...
impl Default for Config {
    fn default() -> Self {
        Self {
            ...
            optimize_working_set: false,
            working_set_min_mb: 0, // 0 = automatic based on total RAM
    ...
    if let Ok(content) = std::fs::read_to_string(&config_path) {
        for line in content.lines() {
            ...
            match key {
                ...
                "WorkingSetMinMB" => if let Ok(v) = val.parse() { config.working_set_min_mb = v; },
...
unsafe fn apply_optimizations() {
    ...
    if config.optimize_working_set {
        ...
        if GlobalMemoryStatusEx(&mut mem_status) != 0 {
            ...
            // Dynamically scale minimum working set:
            // 8GB on 32GB+ systems, 4GB on 16GB+ systems, 2GB on 8GB systems, or custom config
            let min_ws: usize = if config.working_set_min_mb > 0 {
                config.working_set_min_mb * 1024 * 1024
            } else if total_ram >= 28 * 1024 * 1024 * 1024 {
                8 * 1024 * 1024 * 1024 // 8GB for 32GB+ systems
            } else if total_ram >= 14 * 1024 * 1024 * 1024 {
                4 * 1024 * 1024 * 1024 // 4GB for 16GB+ systems
            } else if total_ram >= 7 * 1024 * 1024 * 1024 {
                2 * 1024 * 1024 * 1024 // 2GB for 8GB systems
            } else {
                1024 * 1024 * 1024
            };
